...
export function Login() {
  ...
  if (status === 'success' && result?.approvalState === ORGANIZER_APPROVAL_STATE.approved) {
    // A declarative redirect (not a navigate() call in the render body) -
    // React Router reconciles this like any other render output, and
    // `replace` keeps the login screen out of browser history.
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-qor-canvas">
      <form onSubmit={handleSubmit} className="flex w-80 flex-col gap-4">
        ...
        {status === 'error' && error ? (
          <p role="alert" className="text-sm text-qor-danger">
            {error}
          </p>
        ) : null}
